postgres:
	docker run --name postgres15 -p 5432:5432 -e POSTGRES_USER=cd  -e POSTGRES_PASSWORD=password -d postgres:15-alpine
...
.PHONY: postgres createdb dropdb migrateup migratedown sqlc server new_migration
